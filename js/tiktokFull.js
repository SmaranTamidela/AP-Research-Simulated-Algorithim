...
function logEngagementToSheets(videoObj, metrics) {
  ...
  formData.append("entry.91855232", videoObj.src);          // src
  formData.append("entry.1156097743", videoObj.category);   // category
  formData.append("entry.2074017099", videoObj.username);   // username
  formData.append("entry.826921378", metrics.liked);        // liked
  formData.append("entry.226106810", metrics.favorited);    // favorited
  formData.append("entry.112706924", metrics.watchedPercent); // watchedPercent
  formData.append("entry.1935073296", "fully transparent"); // feedType
...
function chooseNextVideo(currentCategory) {
  ...
  levels.forEach(level => {
    recommendationMap[currentCategory][level].forEach(cat => { if (!candidateCats.includes(cat)) candidateCats.push(cat); });
  });
...
// ---------- VIDEO CARD CREATION ----------
function createVideoCardTransparent(videoObj) {
  ...
  // Log watched percent in 25% increments
  vid.addEventListener("timeupdate", () => {
    if (vid.duration > 0) {
      const percent = Math.min(100, (vid.currentTime / vid.duration) * 100);
      const rounded = Math.floor(percent / 25) * 25;
      if (rounded > metrics.watchedPercent) {
        metrics.watchedPercent = rounded;
...
// ---------- FEED INITIALIZATION ----------
function initTransparentFeed() {
  const feed = document.getElementById("feedContainer");
  const start = randomUnplayedVideo(); playedVideos.add(start.src); feed.appendChild(createVideoCardTransparent(start));
  let current = start;
  window.addEventListener("scroll", () => {
    if (window.innerHeight + window.scrollY >= document.body.offsetHeight - 180) {
      const next = chooseNextVideo(current.category);
      playedVideos.add(next.src);
      feed.appendChild(createVideoCardTransparent(next));
      current = next;
    }
  });
}
