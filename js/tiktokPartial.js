// js/tiktokPartial.js (partial transparency feed with usernames and captions)
...
function chooseNextVideo(currentCategory) {
  ...
  ["high","moderate","low"].forEach(level => {
    recommendationMap[currentCategory][level].forEach(c => { if (!candidateCategories.includes(c)) candidateCategories.push(c); });
  });
...
vid.muted = true;
  ...
  // Recommendation message (partial)
  const recBox = document.createElement("div"); recBox.className="recommendation-box";
  function updateExp() {
    let msg = "This video was recommended to you because you have liked or favorited videos similar to this one.";
    recBox.textContent = msg;
  }
